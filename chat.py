...
def send_message(my_name, input_field):
    ...
    try:
        client.send(input_field.get().encode())
    except:
...
def init(broadcast_window, my_name, name, ip, port):
    global other_name
    global back_window
    other_name = name
    ...
    try:
        ...
        if(data == 'NOTALLOWED'):
            messages_text.config(state=tk.NORMAL)
            messages_text.insert(tk.END, name + ': ' + 'Disconnected\n')
            messages_text.config(state=tk.DISABLED)
            client.close()
        else:
            messages_text.config(state=tk.NORMAL)
        ...
        messagebox.showerror('Timeout', 'Failed to connect to ' + name + ' | ' + ip + ':' + port)
        on_destroy()
